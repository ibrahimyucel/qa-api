...
/*
	...
	File: qa-plugin/social-login/qa-social-login.php
	Version: 3.0.0
...
class qa_social_login {
	...
	function check_login() {
		...
		if($action == 'login') {
			...
			require_once $this->directory . 'qa-sl-utils.php';
	...
	static function printCode($provider, $tourl, $context, $action = 'login', $print = true) {
		...
		$zocial = qa_opt('social_login_zocial') == '1' ? 'zocial' : ''; // use zocial buttons
		if($action == 'logout') {
			$url = $tourl;
			...
			$title = qa_lang_html_sub('social_login/login_using', $provider);
...
HTML;
			...
			$html = <<<HTML
  <a class="social-login-button context-$classes" title="$title" href="$url" rel="nofollow">$text</a>
HTML;
		}
