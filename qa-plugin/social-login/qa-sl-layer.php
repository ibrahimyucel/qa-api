...
/*
	...
	File: qa-plugin/social-login/qa-open-layer.php
	Version: 3.0.0
...
class qa_html_theme_layer extends qa_html_theme_base
{
		...
		if (isset($handle)) {
			...
			if ( in_array($this->template, $tmpl) || $logins_page || 
				...
				$this->content['navigation']['sub']['logins'] = array(
					'label' => qa_lang_html('social_login/my_logins_nav'),
			...
			$title = qa_lang_html('social_login/login_title');
			$descr = qa_lang_html('social_login/login_description');
			
			// hide login/register links from navigation on any page
			if(qa_opt('social_login_hideform') == '1') {
			...
			if(!empty($this->content['custom'])) {
				...
				$this->content['custom'] = "<center><div><p>$descr</p>$content</div></center>";
				
				if($this->content['form'] != null) {
					$this->content['custom'] = "<center><br /><br /><h1>$title</h1>{$this->content['custom']}</center>";
				}
			}
	...
	function head_css() {
		...
		$hidecss = qa_opt('social_login_css') == '1';
		$zocial = qa_opt('social_login_zocial') == '1';
		...
		if (!$hidecss) {
			...
			$this->output(@file_get_contents( QA_HTML_THEME_LAYER_URLTOROOT . 'qa-social-login.css'));
			$this->output('//--></style>');
