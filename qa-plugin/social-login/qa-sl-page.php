...
/*
	...
	File: qa-plugin/social-login/qa-open-page-logins.php
	Version: 3.0.0
...
class qa_social_login_page {
	...
	function process_request($request) {
		...
		if(!$disp_conf) { 
			...
			$qa_content['title'] = qa_lang_html('social_login/my_logins_title');
			...
			$qa_content['title']= qa_lang_html('social_login/other_logins_conf_title');
		...
		if (qa_clicked('dosplit') && !empty($mylogins)) {
			...
			foreach($mylogins as $login) {
				...
				if($key == $unlink) {
					...
					if($useraccount['sessionsource'] != qa_social_login_get_new_source($login['source'], $login['identifier'])) {
	...
	function display_summary(&$qa_content, $useraccount) {
		...
		$qa_content['form_profile']=array(
			'title' => qa_lang_html('social_login/my_current_user'),
			'tags' => 'ENCTYPE="multipart/form-data" METHOD="POST" ACTION="'.qa_self_html().'" CLASS="social-login-profile"',
			'style' => 'wide',
			'fields' => array(
				'handle' => array(
	...
	function display_logins(&$qa_content, $useraccount, $mylogins) {
		if(!empty($mylogins)) {
			require_once $this->directory . 'qa-social-login.php';
			
			// display the logins already linked to this user account
			$qa_content['custom_mylogins']='<h2>' . qa_lang_html('social_login/associated_logins') . '</h2><p>' . qa_lang_html('social_login/split_accounts_note') . '</p>';
			$qa_content['form_mylogins']=array(
				'tags' => 'ENCTYPE="multipart/form-data" METHOD="POST" ACTION="'.qa_self_html().'" CLASS="social-login-accounts"',
				'style' => 'wide',
				'hidden' => array(
					'dosplit' => '1',
				),
			...
			foreach($mylogins as $i => $login) {
				...
				$s = qa_social_login_get_new_source($login['source'], $login['identifier']);
				if($useraccount['sessionsource'] != $s) {
					$del_html = '<a href="javascript://" onclick="OP_unlink(\'' . $login['source'] . '_' . md5($login['identifier']) . '\')" class="opacxdel qa-form-light-button-reject" title="'. qa_lang_html('social_login/unlink_this_account') .'">&nbsp;</a>';
				}
				
				$data["f$i"] = array(
					'label' => qa_social_login::printCode(ucfirst($login['source']), empty($login['ohandle']) ? ucfirst($login['source']) : $login['ohandle'], 'menu', 'view', false) . $del_html,
					'type' => 'static',
					'style' => 'tall'
				);
			}
			...
			$qa_content['customscriptu'] = '<script type="text/javascript">
				function OP_unlink(id) {
					$(".qa-main form.social-login-accounts>input[name=dosplit]").attr("value", id);
					$(".qa-main form.social-login-accounts").submit();
				}
			</script>';
		}
	}
	...
	function display_duplicates(&$qa_content, $useraccount, $otherlogins) {
		...
		if(!empty($otherlogins)) {
			...
			if($disp_conf) {
				$title = '';
				$p = '<br />' . ($disp_conf == 1 ? qa_lang_html('social_login/other_logins_conf_text') : qa_lang_html_sub('social_login/link_exists', '<strong>' . ucfirst(qa_get('link')) . '</strong>') );
			} else {
				$title = '<h2>' . qa_lang_html('social_login/other_logins') . '</h2>';
				$p = qa_lang_html('social_login/other_logins_conf_text');
			}
			
			$qa_content['custom_merge']="$title <p>$p</p>";
			$qa_content['form_merge']=array(
				'tags' => 'ENCTYPE="multipart/form-data" METHOD="POST" ACTION="'.qa_self_html().'" CLASS="social-login-others"',
				'style' => 'wide',
				'buttons' => array(
					'save' => array(
						'tags' => 'onClick="qa_show_waiting_after(this, false);" style="display: none"',
						'note' => qa_lang_html('social_login/action_info_1'),
						'label' => qa_lang_html('social_login/continue'),
					),
				),
				'hidden' => array(
					'domerge' => '1',
			...
			$data = array(); 
			$select = '<option value="0">' . qa_lang_html('social_login/select_base') . '</option>' .
				'<option value="'. $userid . '" title="' . qa_html($useraccount['handle']) .'">' . qa_html($useraccount['handle']) . ' (' . $useraccount['points'] . ' ' . qa_lang_html('admin/points') . ' - ' . qa_lang_html('social_login/current_account') . ')</option>';
			foreach($otherlogins as $i => $login) {
				...
				$login_providers = ($type == 'user' ? strtolower(qa_lang_html('social_login/password')) : '<strong>' . implode(', ', $login['logins']) . '</strong>' );
				
				$data["f$i"] = array(
					'label' => '<strong>' . $name . '</strong> (' . $points . ' ' . qa_lang_html('admin/points') . ', ' . 
						strtolower(qa_lang_html_sub('social_login/login_using', '')) . $login_providers . ')',
			...
			$data['space'] = array(
				'label' => '<br>' . qa_lang_html('social_login/choose_action'),
				'type' => 'static',
				'style' => 'tall'
			);
			
			$ac1html = '<div class="opacxhtml qa-form-tall-buttons" id="ac1html" style="display: none;">' . 
						qa_lang_html('social_login/merge_all_first') . ' ' . qa_lang_html('social_login/merge_note') . '<br /><br />' .
						qa_lang_html('social_login/select_base_note') . '<br /><select name="base1" onchange="OP_baseSelected(this)">' . $select . '</select></div>';
			$ac2html = '<div class="opacxhtml qa-form-tall-buttons" id="ac2html" style="display: none;">' .
						qa_lang_html('social_login/select_merge_first') . ' ' . qa_lang_html('social_login/merge_note') . '<br /><br />' .
						qa_lang_html('social_login/select_base_note') . '<br /><select name="base2" onchange="OP_baseSelected(this)">' . $select . '</select></div>';
			$ac3html = '<div class="opacxhtml qa-form-tall-buttons" id="ac3html" style="display: none;">' . qa_lang_html('social_login/cancel_merge_note') . '</div>';
			
			if($disp_conf == null || $disp_conf == 1) {
				$data['actions1'] = array(
					'label' => ' &nbsp; &nbsp; <strong><a href="javascript:;" onclick="OP_actionSelected(1, \'#ac1html\')">&raquo; ' . qa_lang_html('social_login/merge_all') . '</a></strong>' . $ac1html,
					'type' => 'static',
					'style' => 'tall'
				);
				$data['actions2'] = array(
					'label' => ' &nbsp; &nbsp; <strong><a href="javascript:;" onclick="OP_actionSelected(2, \'#ac2html\')">&raquo; ' . qa_lang_html('social_login/select_merge') . '</a></strong>' . $ac2html,
					'type' => 'static',
					'style' => 'tall'
				);
				if($disp_conf == 1) {
					$data['actions3'] = array(
						'label' => ' &nbsp; &nbsp; <strong><a href="javascript:;" onclick="OP_actionSelected(0, \'#ac3html\')">&raquo; ' . qa_lang_html('social_login/cancel_merge') . '</a></strong>' . $ac3html,
						'type' => 'static',
						'style' => 'tall'
					);
				}
				...
				$data['actions1'] = array(
					'label' => ' &nbsp; &nbsp; <strong><a href="javascript:;" onclick="OP_actionSelected(1, \'#ac1html\')">&raquo; ' . qa_lang_html('social_login/link_all') . '</a></strong>' . $ac1html,
					'type' => 'static',
					'style' => 'tall'
				);
				$data['actions3'] = array(
					'label' => ' &nbsp; &nbsp; <strong><a href="javascript:;" onclick="OP_actionSelected(0, \'#ac3html\')">&raquo; ' . qa_lang_html('social_login/cancel_link') . '</a></strong>' . $ac3html,
					'type' => 'static',
					'style' => 'tall'
				);
			}
			...
			$qa_content['customscript'] = '<script type="text/javascript">
				function OP_actionSelected(i, divid) {
					...
					if(i != 2 || op_last_action == 2) {
						$(".qa-main form.social-login-others input[type=checkbox]").css("visibility", "hidden");
					}
					...
					op_last_action = i;
					$(divid).slideDown();
					$(".qa-main form.social-login-others>input[name=domerge]").attr("value", i);
					
					if(i > 0) {
						$(".qa-main form.social-login-others input[type=checkbox]").attr("checked", "checked");
						if(i == 2) {
							$(".qa-main form.social-login-others input[type=checkbox]").css("visibility", "visible");
							$(".qa-main form.social-login-others select[name=base2]").html( $(".qa-main form.social-login-others select[name=base1]").html() );
						}
						sel = $(".qa-main form.social-login-others select[name=base" + i +"]").get(0);
						sel.selectedIndex = 0;
						OP_baseSelected(sel);
					} else {
						$(".qa-main form.social-login-others span.qa-form-wide-note").html("' . qa_lang_html('social_login/action_info_2') . '"); 
						$(".qa-main form.social-login-others input[type=submit]").show(); 
						$(".qa-main form.social-login-others input[type=submit]").attr("disabled", false);
					}
				}
				...
				function OP_baseSelected(sel) {
					if(!sel || sel.selectedIndex == 0) {
						if(sel) {
							$(".qa-main form.social-login-others span.qa-form-wide-note").html("' . qa_lang_html('social_login/action_info_3') . '")
						} else {
							$(".qa-main form.social-login-others span.qa-form-wide-note").html("' . qa_lang_html('social_login/action_info_1') . '")
						}
						$(".qa-main form.social-login-others input[type=submit]").hide()
						$(".qa-main form.social-login-others input[type=submit]").attr("disabled", "disabled")
					} else {
						if(OP_accValid()) {
							nam = $("option:selected", sel).attr("title")
							$(".qa-main form.social-login-others span.qa-form-wide-note").html("<strong>" + nam + "</strong> '  . qa_lang_html('social_login/action_info_4') . '")
							$(".qa-main form.social-login-others input[type=submit]").show()
							$(".qa-main form.social-login-others input[type=submit]").attr("disabled", false)
						}
					}
				...
				function OP_checkClicked(check) {
					...
					$(".qa-main form.social-login-others select[name=base2]").get(0).selectedIndex = 0
					if(chk) {
						$(".qa-main form.social-login-others select[name=base2] option[value=" + id + "]").show()
					} else {
						$(".qa-main form.social-login-others select[name=base2] option[value=" + id + "]").hide()
					}
					OP_accValid() ') . '
				}
				...
				function OP_accValid() {
					...
					someSel = false 
					$(".qa-main form.social-login-others input[type=checkbox]").each(function(i, o) { 
					...
					if(!someSel) { // nothing selected
						$(".qa-main form.social-login-others span.qa-form-wide-note").html("' . qa_lang_html('social_login/action_info_5') . '");
						return false;
					} else {
						$(".qa-main form.social-login-others span.qa-form-wide-note").html("' . qa_lang_html('social_login/action_info_3') . '");
						return true;
	...
	function display_services(&$qa_content, $has_content) {
		if(!$has_content) {
			...
			$qa_content['form_nodata']=array(
				'title' => '<br>' . qa_lang_html('social_login/no_logins_title'),
				'style' => 'light',
				'fields' => array(
					'note' => array(
						'note' => qa_lang_html('social_login/no_logins_text'),
						'type' => 'static'
					)
				),
			);
		} else {
			$qa_content['custom'] = '<h2>' . qa_lang_html('social_login/link_with_account') . '</h2><p>' . qa_lang_html('social_login/no_logins_text') . '</p>';
		}
		...
		foreach ($loginmodules as $module) {
			ob_start();
			qa_social_login::printCode($module->provider, null, 'associate', 'link');
			$html=ob_get_clean();
	...
	function admin_form() {
		...
		if (qa_clicked('general_save_button')) {
			...
			$hidecss = qa_post_text('social_login_css');
			qa_opt('social_login_css', empty($hidecss) ? 0 : 1);
			
			$zocial = qa_post_text('social_login_zocial');
			qa_opt('social_login_zocial', empty($zocial) ? 0 : 1);
			
			$nologin = qa_post_text('social_login_hideform');
			qa_opt('social_login_hideform', empty($nologin) ? 0 : 1);
			
			$remember = qa_post_text('social_login_remember');
			qa_opt('social_login_remember', empty($remember) ? 0 : 1);
			$saved=true;
		}
		
		$form = array(
			'ok' => $saved ? 'Social Login preferences saved' : null,
			
			'fields' => array(
				array(
					'type' => 'checkbox',
					'label' => 'Don\'t inline CSS. I included the styles in my theme\'s CSS file',
					'value' => qa_opt('social_login_css') ? true : false,
					'tags' => 'NAME="social_login_css"',
				),
				...
				array(
					...
					'value' => qa_opt('social_login_zocial') ? true : false,
					'tags' => 'NAME="social_login_zocial"',
				),
				array(
					'type' => 'checkbox',
					'label' => 'Hide regular login/register forms and keep only external login buttons (might require theme changes)',
					'value' => qa_opt('social_login_hideform') ? true : false,
					'tags' => 'NAME="social_login_hideform"',
				),
				array(
					'type' => 'checkbox',
					...
					'value' => qa_opt('social_login_remember') ? true : false,
					'tags' => 'NAME="social_login_remember"',
				),
				array(
					'type' => 'static',
		...
		foreach($allProviders as $providerFile) {
			...
			if($provider != 'Yahoo') {
				...
				$docUrl = "https://github.com/Jacksiro/q2a-social-login/blob/master/documentation/{$provider}.md";
				$form['fields'][] = array(
					'type' => 'static',
					'label' => '<strong>Tip:</strong> <a href="' . $docUrl . '" target="_blank">Creating a ' . $provider . 
								' APP & get App ID & Secret Key</a>',
				);
			}
