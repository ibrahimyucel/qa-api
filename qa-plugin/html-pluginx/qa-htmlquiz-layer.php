...
class qa_html_theme_layer extends qa_html_theme_base {
	...
	function doctype() {
		...
		$this->pluginurl = qa_opt('site_url').'qa-plugin/html-question/';
	...
	function head_script() {
		...
		if(count((array)$this->extradata) && $this->qa_eqf_file_exist() && qa_opt(qa_htmlquiz::LIGHTBOX_EFFECT)) {
	...
	function qa_eqf_add_field($postid, &$fields, &$form) {
		...
		$multipart = false;
		for($key=qa_htmlquiz::FIELD_COUNT_MAX; $key>=1; $key--) {
			if((bool)qa_opt(qa_htmlquiz::FIELD_ACTIVE.$key)) {
				$field = array();
				$name = qa_htmlquiz::FIELD_BASE_NAME.$key;
				$field['label'] = qa_opt(qa_htmlquiz::FIELD_PROMPT.$key);
				$type = qa_opt(qa_htmlquiz::FIELD_TYPE.$key);
				switch ($type) {
				case qa_htmlquiz::FIELD_TYPE_FILE:
					$field['type'] = 'custom';
					...
					if(!empty($value)) {
						...
						if($width > qa_opt(qa_htmlquiz::THUMB_SIZE))
							$width = qa_opt(qa_htmlquiz::THUMB_SIZE);
						if($format == 'jpg' || $format == 'jpeg' || $format == 'png' || $format == 'gif')
							$original = '<IMG SRC="'.$imageurl.'&qa_size='.$width.'" ALT="'.$filename.'" ID="'.$name.'-thumb" CLASS="'.qa_htmlquiz::FIELD_BASE_NAME.'-thumb"/>';
						$original = '<A HREF="'.$imageurl.'" TARGET="_blank" ID="'.$name.'-link" CLASS="'.qa_htmlquiz::FIELD_BASE_NAME.'-link">' . $original . '</A>';
						$original .= '<INPUT TYPE="checkbox" NAME="'.$name.'-remove" id="'.$name.'-remove" CLASS="'.qa_htmlquiz::FIELD_BASE_NAME.'-remove"/><label for="'.$name.'-remove">'.qa_lang(qa_htmlquiz::PLUGIN.'/html_quiz_remove').'</label><br>';
					...
					break;
				default:
					$field['type'] = qa_opt(qa_htmlquiz::FIELD_TYPE.$key);
					$field['tags'] = 'NAME="'.$name.'"';
					$options = $this->qa_eqf_options(qa_opt(qa_htmlquiz::FIELD_OPTION.$key));
					if (qa_opt(qa_htmlquiz::FIELD_ATTR.$key) != '')
						$field['tags'] .= ' '.qa_opt(qa_htmlquiz::FIELD_ATTR.$key);
					if ($field['type'] != qa_htmlquiz::FIELD_TYPE_TEXT && $field['type'] != qa_htmlquiz::FIELD_TYPE_TEXTAREA)
						$field['options'] = $options;
					if(is_null($postid))
						$field['value'] = qa_opt(qa_htmlquiz::FIELD_DEFAULT.$key);
					else
						$field['value'] = qa_db_single_select(qa_db_post_meta_selectspec($postid, 'qa_q_'.$name));
					if ($field['type'] != qa_htmlquiz::FIELD_TYPE_TEXT && $field['type'] != qa_htmlquiz::FIELD_TYPE_TEXTAREA && is_array($field['options'])) {
						if($field['type'] == qa_htmlquiz::FIELD_TYPE_CHECK) {
							if($field['value'] == 0)
								$field['value'] = '';
						} else
							$field['value'] = @$field['options'][$field['value']];
					}
					if ($field['type'] == qa_htmlquiz::FIELD_TYPE_TEXTAREA) {
						if(isset($options[0]))
							$field['rows'] = $options[0];
						if(empty($field['rows']))
							$field['rows'] = qa_htmlquiz::FIELD_OPTION_ROWS_DFL;
					}
					break;
				}
				$field['note'] = nl2br(qa_opt(qa_htmlquiz::FIELD_NOTE.$key));
				if(isset($qa_extra_question_fields[$name]['error']))
					$field['error'] = $qa_extra_question_fields[$name]['error'];
				$this->qa_eqf_insert_array($fields, $field, $name, qa_opt(qa_htmlquiz::FIELD_FORM_POS.$key));
			}
		}
		if($multipart) {
	...
	function qa_eqf_insert_array(&$items, $insertitem, $insertkey, $findkey) {
		$newitems = array();
		if($findkey == qa_htmlquiz::FIELD_FORM_POS_TOP) {
			$newitems[$insertkey] = $insertitem;
			foreach($items as $key => $item)
				$newitems[$key] = $item;
		} elseif($findkey == qa_htmlquiz::FIELD_FORM_POS_BOTTOM) {
			foreach($items as $key => $item)
				$newitems[$key] = $item;
			$newitems[$insertkey] = $insertitem;
		} else {
			if(!array_key_exists($findkey, $items))
				$findkey = qa_htmlquiz::FIELD_FORM_POS_DFL;
			foreach($items as $key => $item) {
				$newitems[$key] = $item;
				if($key == $findkey)
					$newitems[$insertkey] = $insertitem;
	...
	function qa_eqf_output(&$q_view, $position) {
		...
		foreach($this->extradata as $key => $item) {
			if($item['position'] == $position) {
				...
				if ($type == qa_htmlquiz::FIELD_TYPE_TEXTAREA)
					$value = nl2br($value);
				else if ($type == qa_htmlquiz::FIELD_TYPE_CHECK)
					if ($value == '')
						$value = 0;
				if ($type != qa_htmlquiz::FIELD_TYPE_TEXT && $type != qa_htmlquiz::FIELD_TYPE_TEXTAREA && $type != qa_htmlquiz::FIELD_TYPE_FILE) {
					$options = $this->qa_eqf_options(qa_opt(qa_htmlquiz::FIELD_OPTION.$key));
					if(is_array($options))
						$value = @$options[$value];
				}
				
				if($value == '' && qa_opt(qa_htmlquiz::FIELD_HIDE_BLANK.$key))
					continue;
				
				switch ($position) {
				case qa_htmlquiz::FIELD_PAGE_POS_UPPER:
					...
					break;
				case qa_htmlquiz::FIELD_PAGE_POS_INSIDE:
					...
					break;
				case qa_htmlquiz::FIELD_PAGE_POS_BELOW:
				...
				$title = qa_opt(qa_htmlquiz::FIELD_LABEL.$key);
				if ($type == qa_htmlquiz::FIELD_TYPE_FILE && $value != '') {
				...
				if(qa_opt(qa_htmlquiz::FIELD_PAGE_POS.$key) != qa_htmlquiz::FIELD_PAGE_POS_INSIDE)
					$this->output($output);
				else {
					if(isset($q_view['content'])) {
						$hook = str_replace('^', $key, qa_htmlquiz::FIELD_PAGE_POS_HOOK);
	...
	function qa_eqf_get_extradata($postid) {
		...
		for($key=1; $key<=qa_htmlquiz::FIELD_COUNT_MAX; $key++) {
			if((bool)qa_opt(qa_htmlquiz::FIELD_ACTIVE.$key) && (bool)qa_opt(qa_htmlquiz::FIELD_DISPLAY.$key)) {
				$name = qa_htmlquiz::FIELD_BASE_NAME.$key;
				$value = qa_db_single_select(qa_db_post_meta_selectspec($postid, 'qa_q_'.$name));
				if($value == '' && qa_opt(qa_htmlquiz::FIELD_HIDE_BLANK.$key))
					continue;
				$extradata[$key] = array(
					'name'=>$name,
					'type'=>qa_opt(qa_htmlquiz::FIELD_TYPE.$key),
					'position'=>qa_opt(qa_htmlquiz::FIELD_PAGE_POS.$key),
					'value'=>$value,
				);
			}
		}
	...
	function qa_eqf_file_exist() {
		$fileexist = false;
		foreach($this->extradata as $key => $item) {
			if ($item['type'] == qa_htmlquiz::FIELD_TYPE_FILE)
				$fileexist = true;
		}
		return $fileexist;
	}
