...
class qa_htmlquiz {
	...
	function option_default($option) {
		if ($option==self::FIELD_COUNT) return $this->html_quiz_count;
		if ($option==self::MAXFILE_SIZE) return $this->html_quiz_maxfile_size;
		if ($option==self::ONLY_IMAGE) return $this->html_quiz_only_image;
		if ($option==self::IMAGE_MAXWIDTH) return $this->html_quiz_image_maxwidth;
		if ($option==self::IMAGE_MAXHEIGHT) return $this->html_quiz_image_maxheight;
		if ($option==self::THUMB_SIZE) return $this->html_quiz_thumb_size;
		if ($option==self::LIGHTBOX_EFFECT) return $this->html_quiz_lightbox_effect;
		foreach ($this->extra_fields as $key => $htmlquiz) {
			if ($option==self::FIELD_ACTIVE.$key) return $htmlquiz['active'];
			if ($option==self::FIELD_PROMPT.$key) return $htmlquiz['prompt'];
			if ($option==self::FIELD_NOTE.$key) return $htmlquiz['note'];
			if ($option==self::FIELD_TYPE.$key) return $htmlquiz['type'];
			if ($option==self::FIELD_OPTION.$key) return $htmlquiz['option'];
			if ($option==self::FIELD_ATTR.$key) return $htmlquiz['attr'];
			if ($option==self::FIELD_DEFAULT.$key) return $htmlquiz['default'];
			if ($option==self::FIELD_FORM_POS.$key) return $htmlquiz['form_pos'];
			if ($option==self::FIELD_DISPLAY.$key) return $htmlquiz['display'];
			if ($option==self::FIELD_LABEL.$key) return $htmlquiz['label'];
			if ($option==self::FIELD_PAGE_POS.$key) return $htmlquiz['page_pos'];
			if ($option==self::FIELD_HIDE_BLANK.$key) return $htmlquiz['displayblank'];
			if ($option==self::FIELD_REQUIRED.$key) return $htmlquiz['required'];
		}
	}
	function admin_form(&$qa_content) {
		...
		if (qa_clicked(self::DFL_BUTTON)) {
			...
			foreach ($this->extra_fields as $key => $htmlquiz) {
				qa_opt(self::FIELD_ACTIVE.$key, (int)$htmlquiz['active']);
				qa_opt(self::FIELD_PROMPT.$key, $htmlquiz['prompt']);
				qa_opt(self::FIELD_NOTE.$key, $htmlquiz['note']);
				qa_opt(self::FIELD_TYPE.$key, $htmlquiz['type']);
				qa_opt(self::FIELD_OPTION.$key, $htmlquiz['option']);
				qa_opt(self::FIELD_ATTR.$key, $htmlquiz['attr']);
				qa_opt(self::FIELD_DEFAULT.$key, $htmlquiz['default']);
				qa_opt(self::FIELD_FORM_POS.$key, $htmlquiz['form_pos']);
				qa_opt(self::FIELD_DISPLAY.$key, (int)$htmlquiz['display']);
				qa_opt(self::FIELD_LABEL.$key, $htmlquiz['label']);
				qa_opt(self::FIELD_PAGE_POS.$key, $htmlquiz['page_pos']);
				qa_opt(self::FIELD_HIDE_BLANK.$key, (int)$htmlquiz['displayblank']);
				qa_opt(self::FIELD_REQUIRED.$key, (int)$htmlquiz['required']);
			}
			$this->html_quiz_count = self::FIELD_COUNT_DFL;
			qa_opt(self::FIELD_COUNT,$this->html_quiz_count);
			$this->init_extra_fields($this->html_quiz_count);
			$this->html_quiz_maxfile_size = self::MAXFILE_SIZE_DFL;
			$this->html_quiz_only_image = self::ONLY_IMAGE_DFL;
			$this->html_quiz_image_maxwidth = self::IMAGE_MAXWIDTH_DFL;
			...
			$this->html_quiz_thumb_size = self::THUMB_SIZE_DFL;
			$this->html_quiz_lightbox_effect = self::LIGHTBOX_EFFECT_DFL;
			qa_opt(self::THUMB_SIZE,$this->html_quiz_thumb_size);
			$saved = qa_lang_html(self::PLUGIN.'/'.self::RESET_MESSAGE);
		}
		if ($saved == '' && !$error) {
			$this->html_quiz_count = qa_opt(self::FIELD_COUNT);
			if(!is_numeric($this->html_quiz_count))
				$this->html_quiz_count = self::FIELD_COUNT_DFL;
			$this->init_extra_fields($this->html_quiz_count);
			$this->html_quiz_maxfile_size = qa_opt(self::MAXFILE_SIZE);
			if(!is_numeric($this->html_quiz_maxfile_size))
				$this->html_quiz_maxfile_size = self::MAXFILE_SIZE_DFL;
			$this->html_quiz_image_maxwidth = qa_opt(self::IMAGE_MAXWIDTH);
			if(!is_numeric($this->html_quiz_image_maxwidth))
				$this->html_quiz_image_maxwidth = self::IMAGE_MAXWIDTH_DFL;
			$this->html_quiz_image_maxheight = qa_opt(self::IMAGE_MAXHEIGHT);
			if(!is_numeric($this->html_quiz_image_maxheight))
				$this->html_quiz_image_maxheight = self::IMAGE_MAXHEIGHT_DFL;
			$this->html_quiz_thumb_size = qa_opt(self::THUMB_SIZE);
			if(!is_numeric($this->html_quiz_thumb_size))
				$this->html_quiz_thumb_size = self::THUMB_SIZE_DFL;
		}
		$rules = array();
		foreach ($this->extra_fields as $key => $htmlquiz) {
		...
		foreach ($this->extra_fields as $key => $htmlquiz) {
			...
			$fields[] = array(
				...
				'rows' => $this->html_quiz_note_height,
			...
			$fields[] = array(
				...
				'rows' => $this->html_quiz_option_height,
