...
if ( !defined('QA_VERSION') )
{
	...
	function pm_string_to_array($string_list) 
	{
		$strings = @$string_list;
		if(stripos($strings, '@EVAL') !== false) {
			$strarray = eval(str_ireplace('@EVAL', '', $strings));
			if(empty($strarray)) $strarray = '';
		} else $strarray = str_replace(array("\r\n","\n","\r"), ',', $strings);
		$items = explode(',',$strarray);
		$strarray = array();
		foreach($items as $item) {
			if(strstr($item,'==')) {
				$nameval = explode('==', $item);
				$strarray[$nameval[1]] = trim($nameval[0]);
			} else $strarray[$item] = trim($item);
		}
		return $strarray;
	}
